...
#Environment setting
# Good job to include all the details to install and apply the packages that we need for our assignment!
```{r}
...
# Nice and clear directory to import files to R!
```{r}
...
#############Part I#############
# DO we need to include this in the assignment? I did not do it. It should be OK.
some of the results:
...
#Data inspection
# Nice job to include all the informations we need about the files that we work on.
```{r}
...
#Data processing
# Nice for loop to generate those 40 files!
```{r}
...
# It would be better if you can save the images that you generated.
# It is clear and nice work flow to generate the images!
```{r}
